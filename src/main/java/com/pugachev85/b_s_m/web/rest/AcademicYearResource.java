...
/**
 * REST controller for managing {@link com.pugachev85.b_s_m.domain.AcademicYear}.
 */
@RestController
...
public class AcademicYearResource {
     ...
     */
    @PostMapping("")
    public ResponseEntity<AcademicYear> createAcademicYear(@Valid @RequestBody AcademicYear academicYear) throws URISyntaxException {
    ...
    public ResponseEntity<AcademicYear> partialUpdateAcademicYear(
        @PathVariable(value = "id", required = false) final Long id,
        @NotNull @RequestBody AcademicYear academicYear
    ) throws URISyntaxException {
        ...
        Optional<AcademicYear> result = academicYearRepository
            ...
            .map(existingAcademicYear -> {
                if (academicYear.getTitle() != null) {
                    existingAcademicYear.setTitle(academicYear.getTitle());
                }
