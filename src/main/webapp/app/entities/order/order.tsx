  ...
  return (
    <div>
      <h2 id="order-heading" data-cy="OrderHeading">
        <Translate contentKey="basicStudentsManagmentApp.order.home.title">Orders</Translate>
        <div className="d-flex justify-content-end">
          <Button className="me-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="basicStudentsManagmentApp.order.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to="/order/new" className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="basicStudentsManagmentApp.order.home.createLabel">Create new Order</Translate>
          </Link>
        </div>
      </h2>
        ...
        {orderList && orderList.length > 0 ? (
            ...
            <thead>
              <tr>
                <th className="hand" onClick={sort('id')}>
                  <Translate contentKey="basicStudentsManagmentApp.order.id">ID</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('id')} />
                </th>
                <th className="hand" onClick={sort('title')}>
                  <Translate contentKey="basicStudentsManagmentApp.order.title">Title</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('title')} />
                </th>
                <th className="hand" onClick={sort('date')}>
                  <Translate contentKey="basicStudentsManagmentApp.order.date">Date</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('date')} />
                </th>
                <th className="hand" onClick={sort('number')}>
                  <Translate contentKey="basicStudentsManagmentApp.order.number">Number</Translate>{' '}
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="basicStudentsManagmentApp.order.home.notFound">No Orders found</Translate>
            </div>
          )
        )}
      </div>
