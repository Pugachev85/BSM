  ...
  return (
    <div>
      <h2 id="employee-heading" data-cy="EmployeeHeading">
        <Translate contentKey="basicStudentsManagmentApp.employee.home.title">Employees</Translate>
        <div className="d-flex justify-content-end">
          <Button className="me-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="basicStudentsManagmentApp.employee.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to="/employee/new" className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="basicStudentsManagmentApp.employee.home.createLabel">Create new Employee</Translate>
          </Link>
        </div>
      </h2>
        ...
        {employeeList && employeeList.length > 0 ? (
            ...
            <thead>
              <tr>
                <th className="hand" onClick={sort('id')}>
                  <Translate contentKey="basicStudentsManagmentApp.employee.id">ID</Translate>{' '}
            ...
            <tbody>
              {employeeList.map((employee, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>
                    {employee.academicSubjects
                      ? employee.academicSubjects.map((val, j) => (
                          <span key={j}>
                            <Link to={`/academic-subject/${val.id}`}>{val.title}</Link>
                            {j === employee.academicSubjects.length - 1 ? '' : ', '}
                          </span>
                        ))
                      : null}
                  </td>
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="basicStudentsManagmentApp.employee.home.notFound">No Employees found</Translate>
            </div>
          )
        )}
      </div>
